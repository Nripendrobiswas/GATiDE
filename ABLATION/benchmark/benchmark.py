            ...
            for model_name in models:
              for cur_seed in seeds_list:
                ...
                try:
                    ...
                    # Inject covariate count into any model whose constructor
                    # accepts `num_time_covariates` (GATiDE + variants, and the
                    # cov-aware TiDE used by the ablation study). Models without
                    # the param (dlinear/patchtst/naive) are built unchanged.
                    if use_covariates and split.cov_train is not None:
                        try:
                            _sig = inspect.signature(ModelClass.__init__).parameters
                            if "num_time_covariates" in _sig:
