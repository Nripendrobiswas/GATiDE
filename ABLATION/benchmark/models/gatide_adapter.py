...
from benchmark.models.tide import ResidualBlock
...
class GATiDEPure(nn.Module):
    ...
    Ablation toggles (used by ablation_study/variants.py):
    - use_gate=False -> plain TiDE ResidualBlocks everywhere (no gating)
    - fusion="concat" -> vanilla-TiDE-style concat of projected covariates with
      the flattened lookback instead of SegmentAttentionFusion tokens
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        assert fusion in ("attention", "concat"), f"unknown fusion '{fusion}'"
        self.C = num_features
        self.L = lookback
        self.H = horizon
        ...
        self.fusion = fusion
        self._fusion_concat = fusion == "concat"
        # residual block class: gated (GA-TiDE) vs plain (vanilla TiDE)
        block_cls = GatedResidualBlock if self.use_gate else ResidualBlock
        ...
        if self.num_time_covariates > 0:
            # covariate projection: per-timestep residual block (gated or plain),
            # as in the original _GATideModule (future_cov_projection)
            self.cov_projection = block_cls(
            ...
            cov_flat_dim = (lookback + horizon) * self.temporal_width_future
            if self._fusion_concat:
                # vanilla-TiDE-style pre-mixing: concat projected covs with the
                # flattened lookback (no attention) – the ablation contrast to
                # SegmentAttentionFusion
                self.segment_fusion = None
                fused_dim = lookback * num_features + cov_flat_dim
            else:
                self.segment_fusion = SegmentAttentionFusion(
                    [lookback * num_features, cov_flat_dim], hidden_size, num_attn_heads, dropout)
            ...
            self.segment_fusion = None
            fused_dim = lookback * num_features
        ...
        self.encoders = nn.Sequential(
            block_cls(fused_dim, hidden_size, hidden_size, dropout, use_layer_norm),
            *[block_cls(hidden_size, hidden_size, hidden_size, dropout, use_layer_norm)
        ...
        for _ in range(num_decoder_layers - 1):
            dec_layers.append(block_cls(hidden_size, hidden_size, hidden_size, dropout, use_layer_norm))
        dec_layers.append(block_cls(hidden_size, decoder_output_dim * horizon,
        ...
        self.temporal_decoder = block_cls(
    ...
    def forward(self, x: torch.Tensor, cov: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if self.segment_fusion is not None:
            # attention fusion: lookback + projected covs as attention tokens
            segments = [x.reshape(B, -1), cov_proj.reshape(B, -1)]
            fused = self.segment_fusion(segments)
        elif self._fusion_concat and cov_proj is not None:
            # vanilla-TiDE-style concat pre-mixing (ablation variant)
            fused = torch.cat([x.reshape(B, -1), cov_proj.reshape(B, -1)], dim=1)
        else:
            fused = x.reshape(B, -1)
