...
Compares GA-TiDE against the original TiDE architecture by toggling the two architectural deltas independently, 
under IDENTICAL data protocol, training loop, and hyperparameters:
...
Shared-tuned policy: the hyperparameters tuned for `gatide` (full) via tune_optuna.py are reused for ALL variants 
at each (dataset, horizon) setting, so observed differences are attributable to architecture alone. Falls back to
