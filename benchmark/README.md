...
- **Covariates:** with `--use-covariates`, time-derived covariates (hour, day-of-week, month, day-of-year, + minute if sub-hourly; TiDE §5.1) are generated from the date column and standardized with train-only statistics. They are fed as `(x, y, cov)` window triples into **GATiDE only**, which builds its `SegmentAttentionFusion` with 2 segments — flattened lookback target `(L·C)` and projected covariates `((L+H)·temporal_width_future)` — and feeds the last `H` projected cov steps into the temporal decoder, mirroring `_GATideModule.forward`. Baselines (TiDE/DLinear/PatchTST/Naive) remain covariate-free under this flag — document this asymmetry when reporting. Without the flag GATiDE falls back to the single-segment path (fusion disabled).
